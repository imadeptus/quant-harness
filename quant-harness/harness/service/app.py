"""Hosted verdict service — upload returns, get a calibrated PASS/KILL verdict.

    uvicorn harness.service.app:app --host 0.0.0.0 --port 8000        # Docker / bare
    QH_ROOT_PATH=/api/judge uvicorn harness.service.app:app ...       # behind a prefix

Request flow for ``POST /v1/verdict``: internal secret (401, ``QH_INTERNAL_SECRET``)
-> body-size limit (413, from Content-Length) -> API key (401) -> payment gate
(402, stub) -> schema validation (422) -> matrix limits (413) -> judge
(``harness.audit.audit_returns``, the same function behind ``qh-audit``) -> JSON.
All 401/402/413-by-length decisions come from the headers alone, in ASGI
middleware, before any of the body is read or parsed. One JSON log line per
request goes to stdout.

Mounting: ``Settings.root_path`` (``QH_ROOT_PATH``) is the prefix the app is served
under (``/api/judge`` on Vercel, ``""`` in Docker). Routing follows the ASGI
convention — ``scope["path"]`` is the full path, the prefix is stripped when
present — so both ``/api/judge/healthz`` and ``/healthz`` answer under a prefix,
and every gate keys on the stripped *route path* (``route_path``), never on the
raw one.
"""
...
def route_path(scope: Scope) -> str:
    """The path the router sees: ``scope["path"]`` with ``scope["root_path"]`` stripped
    when present (ASGI convention; the same rule Starlette applies). A request that
    arrives without the prefix routes as-is."""
...
class RequestLogMiddleware:
    """One JSON line per request: route path, status, ms, n_configs, n_periods, verdict
    (plus ``root_path`` when the app is mounted under a prefix)."""
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        try:
            ...
            if root:
                fields["root_path"] = root
            self.logger.info("request", extra={"fields": fields})
...
def _secret_accepted(supplied: Optional[str], secret: str) -> bool:
    return secrets.compare_digest((supplied or "").encode(), secret.encode())
...
class InternalSecretMiddleware:
    """401 for every request except ``/healthz`` unless ``X-Internal-Secret`` equals the
    configured secret (constant-time). The product's Node layer holds the secret;
    nothing else is meant to reach the judge directly. Decided from the headers
    alone, before the body is read. A ``None`` secret makes this a pass-through."""
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        if scope["type"] != "http" or not self.secret or route_path(scope) == HEALTH_PATH:
            await self.app(scope, receive, send)
            return
        request = Request(scope, receive)      # headers only; body untouched
        if not _secret_accepted(request.headers.get(INTERNAL_SECRET_HEADER), self.secret):
            detail = f"missing or invalid {INTERNAL_SECRET_HEADER}"
            response: Response = JSONResponse(status_code=401,
                                              content={"detail": detail, "error": detail},
                                              headers={"WWW-Authenticate": "InternalSecret"})
            await response(scope, receive, send)
            return
        await self.app(scope, receive, send)
...
class AuthGateMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        if scope["type"] != "http" or not route_path(scope).startswith(self.PROTECTED_PREFIX):
            await self.app(scope, receive, send)
            return
...
def compute_verdict(req: VerdictRequest, version: str,
    ...
    metrics = Metrics(
        ...
        path_sharpes_annualized=[v for s in judge["path_sharpes_annualized"]
                                 if (v := _finite_or_none(s)) is not None],
    )
...
def create_app(settings: Settings) -> FastAPI:
    ...
    app = FastAPI(
        ...
        # Mount prefix (QH_ROOT_PATH): FastAPI puts it in scope["root_path"], Starlette
        # strips it when routing, and /docs + openapi.json are prefixed accordingly.
        root_path=settings.root_path,
    )
    app.state.settings = settings
    app.state.payment_gate = gate

    # add_middleware() wraps outermost-last: request log (outer) -> internal secret
    # (401, headers only) -> body-size limit (413 from Content-Length) -> API key /
    # payment gate (401/402, headers only) -> app.
    app.add_middleware(AuthGateMiddleware, api_keys=settings.api_keys, gate=gate)
    app.add_middleware(BodySizeLimitMiddleware, max_bytes=settings.max_body_bytes)
    app.add_middleware(InternalSecretMiddleware, secret=settings.internal_secret)
    app.add_middleware(RequestLogMiddleware, logger=logger)

    api_key_header = APIKeyHeader(name="X-API-Key", auto_error=False)
    internal_secret_header = APIKeyHeader(name=INTERNAL_SECRET_HEADER, auto_error=False)
    ...
    def require_internal_secret(secret: Optional[str] = Security(internal_secret_header)) -> None:
        """Same as above for the internal secret (defence in depth; docs scheme)."""
        if settings.internal_secret and not _secret_accepted(secret, settings.internal_secret):
    ...
    @app.post("/v1/verdict", response_model=VerdictResponse,
              dependencies=[Depends(require_internal_secret), Depends(require_api_key)],
              responses={401: {"description": "missing/invalid internal secret or API key"},
